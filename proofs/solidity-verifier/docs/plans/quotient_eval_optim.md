...
An experimental no-VM quotient CSE mode was added as:

```rust
CodegenConfig {
    quotient_inline_cse: true,
...
A verifier-specific structured-loop mode was added as:

```rust
CodegenConfig {
    quotient_structured_loops: true,
    ..CodegenConfig::default()
}
```
...
An experimental packed-32 mode was added as:

```rust
CodegenConfig {
    quotient_encoding: QuotientProgramEncoding::Packed32,
    quotient_limb_vm_ops: false,
    ..CodegenConfig::default()
}
```
