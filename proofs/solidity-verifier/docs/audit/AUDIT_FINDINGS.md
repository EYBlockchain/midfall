...
| MF-3 | Low | Quotient VM: fold-on-empty re-folds a stale top; native callbacks reset `q_sp` instead of asserting it, hiding dropped operands; u16 const indexes unclamped; `q_sp` unbounded | **Fixed** (guards for each; since then, the u8 const indexes lost their bounded-drift exemption too — every const index is clamped — and native callbacks also fail closed on a live cached top) |
