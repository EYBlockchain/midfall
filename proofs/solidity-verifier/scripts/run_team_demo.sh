...
if [[ -f "$MIDFALL_DIR/Cargo.toml" && -d "$MIDFALL_DIR/aggregation" && -d "$MIDFALL_DIR/curves" ]]; then
  DEFAULT_MOONLIGHT_DIR="$(cd "$MIDFALL_DIR/.." && pwd)/Moonlight"
else
  DEFAULT_MOONLIGHT_DIR="$ROOT_DIR/../Moonlight"
fi
...
Default flow:
  ...
  3. Check or clone the Moonlight checkout.
  4. Compile the trace-only and Moonlight bench paths as a preflight.
  5. Run the IVC Rust/Solidity trace equivalence test without gas checkpoints.
...
Options:
  ...
  --moonlight-dir DIR      Moonlight checkout, relative to this repo unless absolute.
                         Defaults to a Moonlight checkout next to the Midfall repo.
  ...
  --allow-unpinned-solc    Use the configured solc even if it is not the pinned
                           reproducible compiler. Gas/bytecode are ad hoc.
  --rust-toolchain NAME    Export RUSTUP_TOOLCHAIN=NAME for cargo commands
...
Standalone Moonlight bench + trace command:
...
ensure_solc() {
  if env_flag_enabled "$ALLOW_UNPINNED_SOLC"; then
    section "Solidity Compiler"
    explain "Unpinned solc is enabled; generated bytecode and gas are not reproducible."

    local solc_bin="${SOLC:-solc}"
    require_cmd "$solc_bin"
    export SOLC="$solc_bin"
    export HALO2_SOLIDITY_ALLOW_UNPINNED_SOLC=1
    warn "Using unpinned solc: $SOLC ($(solc_version "$SOLC"))"
    return
  fi
...
preflight_ivc_trace() {
  ...
  local args=(--check-only --trace --no-gas-checkpoints --srs-dir "$SRS_DIR")
  if [[ "$SKIP_SRS_DOWNLOAD" -eq 1 ]]; then
    args+=(--skip-srs-download)
  fi

  run_logged \
    "Preflight IVC trace equivalence and SRS assets" \
...
run_trace_equivalence() {
  ...
  local args=(--trace --no-gas-checkpoints --srs-dir "$SRS_DIR")
  if [[ "$SKIP_SRS_DOWNLOAD" -eq 1 ]]; then
    args+=(--skip-srs-download)
  fi
...
print_intro() {
  ...
  if [[ -n "${RUSTUP_TOOLCHAIN:-}" ]]; then
    printf '%s\n' "Rust:       RUSTUP_TOOLCHAIN=$RUSTUP_TOOLCHAIN"
  fi
  printf '\n'
...
while (($#)); do
  case "$1" in
    ...
    --allow-unpinned-solc)
      ...
      ;;
    --rust-toolchain)
      [[ $# -ge 2 ]] || die "--rust-toolchain requires NAME"
      RUST_TOOLCHAIN="$2"
      shift
      ;;
    --no-precheck-builds)
