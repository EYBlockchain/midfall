...
impl<'a> Evaluator<'a> {
    ...
    //   4. (1 - (l_last + l_blind)) * (left_i - right_i) [for each set] where
    //      left_i  = z_i_next * ∏_{c in chunk_i} (eval(c) + β·s(c) + γ) right_i =
    //      z_i      * ∏_{c in chunk_i} (eval(c) + δ_pow + γ) δ_pow   starts at
    //      β·x·δ^(i*chunk_len) and is *= δ each step
    //
    ...
    pub(crate) fn permutation_computations(&self) -> Vec<(Vec<String>, String)> {
        ...
        for i in 1..self.meta.num_permutation_zs {
            ...
            let z_prev_last =
                z_evals[i - 1].2.as_ref().expect("non-last set has last eval").to_string();
            let l0 = self.fresh_var();
            lines.push(format!("let {l0} := mload(L_0_MPTR)"));
