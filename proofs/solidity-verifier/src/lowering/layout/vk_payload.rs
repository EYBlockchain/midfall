...
use crate::lowering::layout::{G1_WORDS, WORD_BYTES};
...
impl PackedProgramCodec {
    ...
    pub(crate) fn encode_words(bytes: &[u8]) -> Vec<U256> {
        ...
        padded.chunks(WORD_BYTES).map(U256::from_be_slice).collect::<Vec<_>>()
    }
...
mod tests {
    ...
    fn vk_payload_layout_reserves_monotonic_sections() {
        ...
        assert_eq!(
            layout.word_offset(PayloadSectionKind::PermutationCommitments).unwrap(),
            67
        );
        assert_eq!(layout.total_words(), 75);
        assert_eq!(
            layout.byte_offset(PayloadSectionKind::FixedCommitments).unwrap(),
            39 * 0x20
        );
        assert_eq!(
            layout.byte_len(PayloadSectionKind::PermutationCommitments).unwrap(),
            2 * 0x80
        );
    }
    ...
    fn vk_payload_layout_rejects_duplicate_sections() {
        ...
        layout
            .reserve(
                ...
                crate::lowering::layout::VK_HEADER_WORDS,
            )
            .unwrap();
        let err = layout
