...
mod tests {
    ...
    fn quotient_program_artifacts_match_typed_identity_stream() {
        ...
        let selector_fold = SolidityGenerator::selector_fold_plan(&identities, 2);
        assert_eq!(
        ...
        for identity in &identities {
            builder.identity_expr(
                identity.expr.as_ref().expect("synthetic identity has expr"),
                identity.target,
                selector_fold.gap_for(identity),
                None,
            );
        }
        validate_quotient_program(&builder.bytes, QuotientProgramEncoding::Bytes)
            .expect("synthetic quotient program should validate");
    ...
    fn quotient_program_items_emit_bytecode_markers_in_manifest_order() {
        ...
        let selector_fold = SolidityGenerator::selector_fold_plan(&[interpreted.clone()], 1);
        let mut builder = QuotientProgramBuilder::default();
        ...
        builder.identity_expr(
            interpreted
                .expr
                .as_ref()
                .expect("synthetic identity has expr"),
            interpreted.target,
    ...
    fn selector_sparse_fold_exhaustive_matches_naive_global_y_powers() {
        ...
        for len in 1..=8usize {
            ...
            for mut case in 0..cases {
                ...
                let expected =
                    expected_linearization_artifacts_for_test(&identities, &values, y, 2);

                let mut sparse_main = Fq::ZERO;
                let mut sparse_selectors = vec![Fq::ZERO; 2];
                for identity in &identities {
                    let value = eval_quotient_expr_for_test(
                        identity.expr.as_ref().expect("synthetic identity has expr"),
                        &values,
                    );
                    match identity.target {
                        ...
                        QuotientTarget::Selector(selector_idx) => {
                            ...
                            let gap = selector_fold
                                .gap_for(identity)
                                .expect("selector identity has gap");
                            sparse_selectors[selector_idx] =
                                sparse_selectors[selector_idx] * fq_pow(y, gap) + value;
                        }
                    }
                }
                for (bucket, tail) in sparse_selectors
                    .iter_mut()
                    .zip(selector_fold.tail_exponents.iter())
                {
    ...
    fn test_quotient_execution_plan(
        ...
        let selector_count = expected
            ...
            .max()
            .unwrap_or(0);
        let has_native_permutation = items
            .iter()
            .any(|item| matches!(item, QuotientProgramItem::NativePermutation));
        let has_native_lookup = items
            .iter()
            .any(|item| matches!(item, QuotientProgramItem::NativeLookup));
        QuotientProgramPlan {
            inline_identities,
            items,
            native_identities,
            ...
            has_native_lookup,
            selector_fold: SolidityGenerator::selector_fold_plan(&expected, selector_count),
        }
    }
