...
//! EVM-facing helpers for calldata encoding, Solidity compilation, and revm
//! tests.
//!
//! Production callers use the calldata encoders; the `evm` feature also
//! exposes pinned-solc compilation and a small revm harness for verifier smoke
//! tests.

use ff::PrimeField;
use itertools::chain;

use crate::lowering::encoding::{fe_to_u256, to_u256_be_bytes};
...
pub(crate) mod test {
    ...
    /// Environment flag that lets ad-hoc benches use the configured `solc`
    /// even when it does not match [`PINNED_SOLC_VERSION`].
    ///
    /// This is intentionally opt-in because generated bytecode and gas numbers
    /// are only reproducible with the pinned compiler.
    pub const ALLOW_UNPINNED_SOLC_ENV: &str = "HALO2_SOLIDITY_ALLOW_UNPINNED_SOLC";
    ...
    fn allow_unpinned_solc() -> bool {
        env_flag_enabled(ALLOW_UNPINNED_SOLC_ENV)
    }
    ...
    /// version/commit, or when [`ALLOW_UNPINNED_SOLC_ENV`] explicitly allows
    /// ad-hoc non-reproducible compilation.
    pub fn pinned_solc_available() -> bool {
        match solc_version() {
            Ok(version) if version.starts_with(PINNED_SOLC_VERSION) => true,
            Ok(version) if allow_unpinned_solc() => {
                eprintln!(
                    "configured solc version {version} does not match pinned {PINNED_SOLC_VERSION}; \
                     continuing because {ALLOW_UNPINNED_SOLC_ENV}=1"
                );
                true
            }
            Ok(version) => {
    ...
    /// Return the configured solc path after enforcing the pinned compiler,
    /// unless [`ALLOW_UNPINNED_SOLC_ENV`] explicitly allows ad-hoc compilation.
    fn require_pinned_solc() -> String {
        let solc = solc_command();
        let version = solc_version()
            .unwrap_or_else(|err| panic!("Command '{solc}' not found or unusable: {err}"));
        if allow_unpinned_solc() && !version.starts_with(PINNED_SOLC_VERSION) {
            eprintln!(
                "warning: compiling with unpinned solc `{solc}` version `{version}`; \
                 bytecode and gas are not reproducible against pinned {PINNED_SOLC_VERSION}"
            );
            return solc;
        }
        assert!(
    ...
    /// [`PINNED_SOLC_VERSION`] without [`ALLOW_UNPINNED_SOLC_ENV`], or
    /// compilation fails.
    ...
    impl Debug for Evm {
        /// Hide the in-memory revm database from debug output while still
        /// satisfying the crate-wide `missing_debug_implementations` lint.
    ...
    impl Evm {
        ...
        /// Apply create transaction with an address constructor argument
        ...
        pub fn create_with_address_arg(
            ...
            bytecode.extend_from_slice(
                &U256::try_from_be_slice(address_arg.as_slice()).unwrap().to_be_bytes::<0x20>(),
            );
        ...
        pub fn create_with_two_address_args(
            ...
            for address in [first_address_arg, second_address_arg] {
                bytecode.extend_from_slice(
                    &U256::try_from_be_slice(address.as_slice()).unwrap().to_be_bytes::<0x20>(),
                );
            }
            self.create(bytecode)
        }
        ...
        /// Apply call transaction and return gas, return data, and emitted
        /// logs.
