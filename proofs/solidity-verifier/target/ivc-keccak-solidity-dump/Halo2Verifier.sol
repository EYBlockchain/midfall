...
contract Halo2Verifier {
    ...
    uint256 internal constant EXPECTED_QUOTIENT_LENGTH = 9927;
    uint256 internal constant EXPECTED_QUOTIENT_CODEHASH_WORD = 0xee79ea26278dda9e5372ea2c38d3f80610c0fe7c002359c7b638f1556f3c4cc9;
    ...
    function verifyProof(
        ...
        assembly ("memory-safe") {
            ...
            let lagrange_precompile_failed := 0
            {
                ...
                // 14 instance + 10 negative-rotation
            ...
            // ===============================================================
            {
                ...
                // grouped so gas checkpoints can attribute their cost.
                {
                    ...
                    mcopy(add(PCS_FINAL_MSM_MPTR, 0xd20), 0x6c00, 0x80)
                    ...
                    // term 49: fixed_comms[20] (simple selector 4)
                    mcopy(add(PCS_FINAL_MSM_MPTR, 0x1ea0), 0x6c80, 0x80)
                    mstore(add(PCS_FINAL_MSM_MPTR, 0x1f20), mulmod(lin_query_scalar_41, mload(add(SELECTOR_ACC_MPTR, 0x80)), r))
                    // term 50: fixed_comms[21] (simple selector 5)
                    mcopy(add(PCS_FINAL_MSM_MPTR, 0x1f40), 0x6d00, 0x80)
                    mstore(add(PCS_FINAL_MSM_MPTR, 0x1fc0), mulmod(lin_query_scalar_41, mload(add(SELECTOR_ACC_MPTR, 0xa0)), r))
                    // term 51: fixed_comms[22] (simple selector 6)
                    mcopy(add(PCS_FINAL_MSM_MPTR, 0x1fe0), 0x6d80, 0x80)
                    mstore(add(PCS_FINAL_MSM_MPTR, 0x2060), mulmod(lin_query_scalar_41, mload(add(SELECTOR_ACC_MPTR, 0xc0)), r))
                    // term 52: fixed_comms[23] (simple selector 7)
                    mcopy(add(PCS_FINAL_MSM_MPTR, 0x2080), 0x6e00, 0x80)
                    mstore(add(PCS_FINAL_MSM_MPTR, 0x2100), mulmod(lin_query_scalar_41, mload(add(SELECTOR_ACC_MPTR, 0xe0)), r))
                    // term 53: fixed_comms[24] (simple selector 8)
                    mcopy(add(PCS_FINAL_MSM_MPTR, 0x2120), 0x6e80, 0x80)
